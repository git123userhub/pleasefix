...
class DynamicRect:
    def __new__(cls, surface, rect, colors, antialias=True, orient="horizontal"):
        if isinstance(colors[0], int) or common.is_str(colors) or isinstance(colors, pygame.Color):
            return RoundRect(surface, rect, colors, antialias, orient)
        elif isinstance(colors[0], (list, tuple, pygame.Color, str)) and len(colors) == 2:
            return RoundRectGrad2(surface, rect, *colors, antialias, orient)
        elif isinstance(colors[0], (list, tuple, pygame.Color, str)) and len(colors) == 3:
            return RoundRectGrad3(surface, rect, *colors, antialias, orient)
        else:
